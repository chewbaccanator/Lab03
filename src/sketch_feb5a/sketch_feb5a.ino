...
const int turnSpeed = 100; // manual pivot turn speed

// Track commanded motor values (signed)
int currentLeftCmd  = 0;   // -255..255s
...
// ---------- Telemetry timing ----------
const unsigned long telemetryPeriodMs = 1000; // 50 Hz
...
void updateEncodersByPolling() {
  ...
  // Count any transition (HIGH->LOW or LOW->HIGH)
  if (a != lastEncLeft)  leftTicks++;
  if (b != lastEncRight) rightTicks++;
...
void handleSerialCommands() {
  ...
  } else if (c == 's') { // backward  <-- THIS IS THE KEY CHANGE
    manualLeftCmd  = -(baseSpeed + leftTrim);
    manualRightCmd = -(baseSpeed + rightTrim);
